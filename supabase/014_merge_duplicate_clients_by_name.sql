...
begin
  ...
  update public.clients as canonical
  set instagram_handle = duplicate_client_merged_values.instagram_handle
  from duplicate_client_merged_values
  where canonical.id = duplicate_client_merged_values.canonical_id
    and canonical.instagram_handle is null
    and duplicate_client_merged_values.instagram_handle is not null
    and not exists (
      select 1
      from public.clients as other_client
      where other_client.id <> canonical.id
        and other_client.instagram_handle = duplicate_client_merged_values.instagram_handle
    );
